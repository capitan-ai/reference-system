  ...
  try {
    ...
    try {
      ...
      await saveApplicationLog(prisma, {
        ...
        logId: `gc-create-fail-${customerId || customerName || 'unknown'}-${Date.now()}`,
        status: 'error',
        payload: {
          stage: 'create',
          ...
          square_error: createError.errors || createError.message
        }
      })
      throw createError
    }
    ...
    if (!createResponse.result.giftCard) {
      ...
      await saveApplicationLog(prisma, {
        organizationId,
        logType: 'SQUARE_GIFT_CARD_API',
        logId: `gc-create-empty-${customerId || customerName || 'unknown'}-${Date.now()}`,
        status: 'error',
        payload: {
          stage: 'create-empty-response',
          customer_id: customerId,
          customer_name: customerName,
          is_referrer: isReferrer
        }
    ...
    if (locationId && orderInfo?.orderId && orderInfo?.lineItemUid) {
      ...
      try {
        ...
        await saveApplicationLog(prisma, {
          organizationId,
          logType: 'SQUARE_GIFT_CARD_API',
          logId: `gc-activate-order-fail-${giftCardId}-${Date.now()}`,
          status: 'error',
          payload: {
            stage: 'activate-order',
            gift_card_id: giftCardId,
            customer_id: customerId,
            order_id: orderInfo?.orderId,
            line_item_uid: orderInfo?.lineItemUid,
            square_error: activateError.errors || activateError.message
          }
        })
      }
    }
      ...
      try {
        ...
        if (activity) {
          ...
          await saveApplicationLog(prisma, {
            organizationId,
            logType: 'SQUARE_GIFT_CARD_API',
            logId: `gc-activate-owner-empty-${giftCardId}-${Date.now()}`,
            status: 'error',
            payload: {
              stage: 'activate-owner-empty-response',
              gift_card_id: giftCardId,
              customer_id: customerId,
        ...
        await saveApplicationLog(prisma, {
          organizationId,
          logType: 'SQUARE_GIFT_CARD_API',
          logId: `gc-activate-owner-fail-${giftCardId}-${Date.now()}`,
          status: 'error',
          payload: {
    ...
    if (!giftCardActivity && locationId && amountMoney.amount > 0) {
      ...
      try {
        ...
        await saveApplicationLog(prisma, {
          organizationId,
          logType: 'SQUARE_GIFT_CARD_API',
          logId: `gc-adjust-fail-${giftCardId}-${Date.now()}`,
          status: 'error',
          payload: {
            stage: 'adjust-increment',
            gift_card_id: giftCardId,
            customer_id: customerId,
            amount_cents: amountMoney.amount,
            square_error: adjustError.errors || adjustError.message
          }
        })
      }
    }
    ...
    await saveApplicationLog(prisma, {
      organizationId,
      logType: 'SQUARE_GIFT_CARD_API',
      logId: `gc-create-outer-fail-${customerId || customerName || 'unknown'}-${Date.now()}`,
      status: 'error',
      payload: {
        stage: 'create-gift-card-outer',
        customer_id: customerId,
        customer_name: customerName,
        is_referrer: isReferrer,
        amount_cents: amountCents,
        error_message: error.message,
        square_error: error.errors || null
      }
    })
    return null
  }
...
async function loadGiftCard(
  ...
  try {
    ...
    if (customerId) {
      try {
        const customerOrgRow = await prisma.$queryRaw`
          SELECT organization_id FROM square_existing_clients
          WHERE square_customer_id = ${customerId}
          LIMIT 1
        `
        loadOrgId = customerOrgRow?.[0]?.organization_id || null
    ...
    if (cardState === 'PENDING') {
      ...
      try {
        ...
        console.error(`❌ loadGiftCard ACTIVATE failed for ${giftCardId}:`, activateError.message)
        if (activateError.errors) {
          console.error(`Square API errors:`, safeStringify(activateError.errors))
        }
        await saveApplicationLog(prisma, {
          organizationId: loadOrgId,
          logType: 'SQUARE_GIFT_CARD_API',
          logId: `gc-load-activate-fail-${giftCardId}-${Date.now()}`,
          status: 'error',
          payload: {
            stage: 'load-activate',
            gift_card_id: giftCardId,
      ...
      try {
        ...
        console.error(`❌ loadGiftCard ADJUST_INCREMENT failed for ${giftCardId}:`, adjustError.message)
        if (adjustError.errors) {
          console.error(`Square API errors:`, safeStringify(adjustError.errors))
        }
        await saveApplicationLog(prisma, {
          organizationId: loadOrgId,
          logType: 'SQUARE_GIFT_CARD_API',
          logId: `gc-load-adjust-fail-${giftCardId}-${Date.now()}`,
          status: 'error',
          payload: {
            stage: 'load-adjust',
            gift_card_id: giftCardId,
        ...
        return { success: false, error: adjustError.message }
      }
    }
    ...
    await saveApplicationLog(prisma, {
      organizationId: null,
      logType: 'SQUARE_GIFT_CARD_API',
      logId: `gc-load-outer-fail-${giftCardId}-${Date.now()}`,
      status: 'error',
      payload: {
        stage: 'load-gift-card-outer',
        gift_card_id: giftCardId,
        customer_id: customerId,
        amount_cents: amountCents,
        context_label: contextLabel,
  ...
  try {
    ...
    if (!customerId) {
      ...
      await saveApplicationLog(prisma, {
        organizationId: null,
        logType: 'REWARD_SKIP',
        logId: `skip-no_customer_id-${paymentId || 'unknown'}-${Date.now()}`,
        status: 'completed',
        payload: {
          reason: 'no_customer_id',
          payment_id: paymentId,
          correlation_id: runContext?.correlationId || null
        }
      })
      return
    }
    ...
    if (!organizationId) {
      ...
      await saveApplicationLog(prisma, {
        organizationId: null,
        logType: 'REWARD_SKIP',
        logId: `skip-no_org_id-${customerId}-${paymentId || 'unknown'}-${Date.now()}`,
        status: 'completed',
        payload: {
          reason: 'no_org_id',
          customer_id: customerId,
          payment_id: paymentId,
          payment_location_id: paymentLocationId,
          correlation_id: runContext?.correlationId || null
        }
      })
      return
    ...
    if (!customerData || customerData.length === 0) {
      ...
      await saveApplicationLog(prisma, {
        organizationId,
        logType: 'REWARD_SKIP',
        logId: `skip-customer_not_in_db-${customerId}-${paymentId || 'unknown'}-${Date.now()}`,
        status: 'completed',
        payload: {
          reason: 'customer_not_in_db',
          customer_id: customerId,
          payment_id: paymentId,
          correlation_id: runContext?.correlationId || null
        }
    ...
    if (customer.first_payment_completed) {
      ...
      await saveApplicationLog(prisma, {
        organizationId,
        logType: 'REWARD_SKIP',
        logId: `skip-already_completed-${customerId}-${paymentId || 'unknown'}-${Date.now()}`,
        status: 'completed',
        payload: {
