...
import { fetchAcquisitionSourceFromSquare, saveAcquisitionSourceIfMissing } from '../../../../lib/utils/acquisition-source.js'
...
export async function processCustomerCreated(payload, eventId, eventCreatedAt) {
  ...
  try {
    ...
    try {
      const source = await fetchAcquisitionSourceFromSquare(customerId)
      if (source) {
        await saveAcquisitionSourceIfMissing(prisma, organizationId, customerId, source)
        console.log(`[WEBHOOK-PROCESSOR] 📊 Captured acquisition_source: "${source}"`)
      }
    } catch (err) {
      console.warn(`[WEBHOOK-PROCESSOR] ⚠️ Failed to fetch acquisition_source: ${err.message}`)
    }
