...
bookings_agg AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE b_inner.status LIKE 'CANCELLED%' OR b_inner.status = 'NO_SHOW') AS b_cancelled_count
...
final_data AS (
  SELECT
    ...
    CASE
      ...
      WHEN COALESCE(b.b_cancelled_count,0) > 0 THEN 'CANCELLED_ONLY'
      ELSE 'POTENTIAL'
