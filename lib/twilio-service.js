...
async function sendReferralCodeSms({
  ...
  if (!normalizedTo) {
    ...
    await recordNotificationEvent({
      ...
      metadata: { referralUrl, name, ...metadata }
    })
  ...
  if (!smsReady || !twilioClient) {
    ...
    await recordNotificationEvent({
      ...
      referrerCustomerId,
      referralEventId,
      metadata: { referralUrl, name, simulated: true, reason: 'sms-disabled', ...metadata }
    })
  ...
  try {
    ...
    await recordNotificationEvent({
      ...
      templateId: 'TWILIO_REFERRAL_SMS',
      sentAt: new Date(),
      metadata: { referralUrl, name, ...metadata }
    })
