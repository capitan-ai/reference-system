  ...
  const refreshSQL = `
    ...
    rewards_agg AS (
      SELECT
        ...
        COUNT(DISTINCT COALESCE(sec.personal_code, rp.personal_code))::int AS unique_codes_rewarded,
        COUNT(*)::int AS referrer_rewards_count,
    ...
    -- 5. Redemptions (gift_card_transactions REDEEM)
    redemptions_agg AS (
      SELECT
        gc.organization_id,
        (t.created_at AT TIME ZONE 'America/Los_Angeles')::date AS date_pacific,
        COUNT(*)::int AS redemptions_count,
        COALESCE(SUM(ABS(t.amount_cents)), 0)::int AS redemptions_cents
