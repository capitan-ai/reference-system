 ...
 *
 * Pacific calendar date — pattern depends on column type:
 *   - timestamptz columns (e.g. customer_analytics.first_visit_at, referral_rewards.created_at,
 *     gift_card_transactions.created_at):
 *       (col AT TIME ZONE 'America/Los_Angeles')::date
 *   - timestamp-without-tz columns storing UTC (e.g. notification_events.createdAt/sentAt,
 *     device_pass_registrations."createdAt"):
 *       (col AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')::date
 *   Using the wrong one shifts the date 7–8 hours in the wrong direction.
 */
...
async function refreshReferralAnalytics(db, dateFrom, dateTo) {
  const refreshSQL = `
    ...
    new_customers_agg AS (
          ...
          json_agg(
            json_build_object(
              ...
              'referrer_name', COALESCE(
                NULLIF(TRIM(COALESCE(sec_ref.given_name, '') || ' ' || COALESCE(sec_ref.family_name, '')), ''),
                NULLIF(TRIM(COALESCE(sec_ref2.given_name, '') || ' ' || COALESCE(sec_ref2.family_name, '')), '')
              ),
      ...
      -- Resolve referrer name: find code owner via referral_profiles
      LEFT JOIN referral_profiles rp_ref
        ON rp_ref.organization_id = ca.organization_id
        AND (LOWER(COALESCE(rp_ref.personal_code, '')) = LOWER(sec.used_referral_code)
             OR LOWER(COALESCE(rp_ref.referral_code, '')) = LOWER(sec.used_referral_code))
        AND rp_ref.square_customer_id <> ca.square_customer_id
      LEFT JOIN square_existing_clients sec_ref
        ON sec_ref.organization_id = ca.organization_id
        AND sec_ref.square_customer_id = rp_ref.square_customer_id
      -- Fallback: find code owner directly via square_existing_clients.personal_code
      LEFT JOIN square_existing_clients sec_ref2
        ON sec_ref2.organization_id = ca.organization_id
        AND LOWER(COALESCE(sec_ref2.personal_code, '')) = LOWER(sec.used_referral_code)
        AND sec_ref2.square_customer_id <> ca.square_customer_id
        AND rp_ref.square_customer_id IS NULL
    ...
    emails_agg AS (
      SELECT
        organization_id,
        (COALESCE("sentAt", "createdAt") AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')::date AS date_pacific,
      ...
      GROUP BY organization_id, (COALESCE("sentAt", "createdAt") AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')::date
    ),
    ...
    wallet_agg AS (
      SELECT
        gc.organization_id,
        (dpr."createdAt" AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')::date AS date_pacific,
      ...
      GROUP BY gc.organization_id, (dpr."createdAt" AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles')::date
    ),
