...
export async function processDiscountAdjustments(organizationId) {
  ...
  try {
    ...
    // Master pays for: complaints, delays, late arrivals, no-shows
    // TODO: Add material cost patterns when salon starts tracking materials via discount names
    const MASTER_PAYS_PATTERNS = ['complaint', 'delay', 'late', 'waiting', 'no-show', 'no show', 'noshow', 'опоздание', 'неявка', 'штраф']
    const DEFAULT_OWNER_RULE = { master_share_percent: 0, salon_share_percent: 100 }

    function resolveSingleRule(key) {
      // 1. Exact match
      ...
      // 3. Default: owner pays
      return DEFAULT_OWNER_RULE
    }

    function resolveRule(discountName) {
      const key = discountName.toLowerCase().trim()
      // Handle combo discounts like "Review, Pacific10"
      if (key.includes(',')) {
        const parts = key.split(',').map((p) => p.trim()).filter(Boolean)
        let best = DEFAULT_OWNER_RULE
        for (const part of parts) {
          const rule = resolveSingleRule(part)
          if (rule.master_share_percent > best.master_share_percent) {
            best = rule
          }
        }
        return best
      }
      return resolveSingleRule(key)
    }
