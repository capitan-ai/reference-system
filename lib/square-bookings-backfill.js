...
class SquareBookingsBackfill {
  constructor(prisma, squareClient, locationId, options = {}) {
    ...
    this.bookingsApi = squareClient.bookings
    this.customerId = options.customerId || null // Optional customer ID filter
    this.organizationId = options.organizationId || null // Organization ID (will be resolved from locationId if not provided)
  ...
  /**
   * Resolve organization_id from location_id (square_location_id)
   * Caches the result in this.organizationId
   */
  async resolveOrganizationId() {
    ...
    if (!this.locationId) {
      throw new Error('Cannot resolve organization_id: locationId is required')
    }

    try {
      const location = await this.prisma.$queryRaw`
        SELECT organization_id FROM locations 
        WHERE square_location_id = ${this.locationId} 
        LIMIT 1
      `
      
      if (location && location.length > 0 && location[0].organization_id) {
        this.organizationId = location[0].organization_id
        return this.organizationId
      }

      throw new Error(`Organization not found for location_id: ${this.locationId}`)
    } catch (error) {
      throw new Error(`Failed to resolve organization_id from location_id ${this.locationId}: ${error.message}`)
    }
  }
  ...
  async fetchBookingsPage(cursor = null, updatedAfter = null, startAtMin = null, startAtMax = null) {
    ...
    while (attempt < this.maxRetries) {
      try {
        ...
        // Use axios for direct HTTP call since SDK positional parameters are unreliable
        const envName = process.env.SQUARE_ENVIRONMENT || 'production'
        const baseUrl = envName === 'sandbox' 
          ? 'https://connect.squareupsandbox.com/v2' 
          : 'https://connect.squareup.com/v2'
        const accessToken = process.env.SQUARE_ACCESS_TOKEN
        
        const axios = require('axios')
        const httpResponse = await axios.get(`${baseUrl}/bookings`, {
          params: {
            limit: this.limit,
            cursor: cursor || undefined,
            location_id: this.locationId,
            customer_id: this.customerId || undefined,
            start_at_min: startAtParam,
            start_at_max: endAtParam
          },
          headers: {
  ...
  async upsertBooking(booking) {
    ...
    try {
      ...
      if (customerId) {
        try {
          // Resolve organization_id from location_id (required for multi-tenant)
          const organizationId = await this.resolveOrganizationId()
          
          // Check if customer exists using composite unique constraint
          ...
          if (!existingCustomer) {
            ...
            try {
              const customerResponse = await this.square.customers.get(customerId)
                    ...
                    try {
                      await this.prisma.$executeRaw`
                        INSERT INTO square_existing_clients (
                          organization_id, square_customer_id, given_name, family_name, 
                          email_address, phone_number, got_signup_bonus, 
                          created_at, updated_at
                        ) VALUES (
                          ${organizationId}::uuid, ${customerId}, ${squareCustomer.givenName || null}, 
                ...
                try {
                  ...
                  try {
                    await this.prisma.$executeRaw`
                      ...
                      ) VALUES (${organizationId}::uuid, ${customerId}, false, NOW(), NOW())
                      ON CONFLICT (organization_id, square_customer_id) DO NOTHING
                    `
                    customerCreated = true
                  } catch (insertError) {
      ...
      // Resolve administrator_id (creator team member) to internal UUID
      let administratorUuid = null
      if (creatorType === 'TEAM_MEMBER' && creatorTeamMemberId) {
        try {
          const organizationId = await this.resolveOrganizationId()
          const adminRecord = await this.prisma.$queryRaw`
          ...
          if (administratorUuid) {
            console.log(`   ✅ Resolved administrator ${creatorTeamMemberId} → ${administratorUuid}`)
          }
        } catch (error) {
      ...
      try {
        ...
        INSERT INTO bookings (
          ...
          ${creatorType}, ${creatorCustomerId}, ${administratorUuid},
      ...
      // Process appointment segments - save each as a separate booking record
      ...
      if (segments.length === 0) {
        // No services, booking already saved above
        this.stats.totalUpserted++
        return true
      }

      // Pre-resolve all segment technician IDs from Square IDs to internal UUIDs
      const resolvedTechnicianIds = {}
      const orgId = await this.resolveOrganizationId()
      for (const segment of segments) {
        const sqTeamMemberId = segment.team_member_id || segment.teamMemberId
        if (sqTeamMemberId && !resolvedTechnicianIds[sqTeamMemberId]) {
          try {
            const tmRecord = await this.prisma.$queryRaw`
              SELECT id::text as id FROM team_members
              WHERE square_team_member_id = ${sqTeamMemberId}
                AND organization_id = ${orgId}::uuid
              LIMIT 1
            `
            resolvedTechnicianIds[sqTeamMemberId] = tmRecord && tmRecord.length > 0 ? tmRecord[0].id : null
          } catch (error) {
            resolvedTechnicianIds[sqTeamMemberId] = null
          }
        }
      }
      ...
      for (const segment of segments) {
        const segmentBookingId = segments.length > 1 
          ? `${bookingId}-${segment.service_variation_id || segment.serviceVariationId || 'unknown'}`
          : bookingId
        
        // Ensure service variation exists
        ...
        // Save booking with service fields
        try {
          await this.prisma.$executeRaw`
            INSERT INTO bookings (
              id, version, customer_id, location_id, location_type, source,
              ...
              ${resolvedTechnicianIds[segment.team_member_id || segment.teamMemberId] || null},
              ${segment.any_team_member ?? segment.anyTeamMember ?? false},
              ${createdAtSql}::timestamptz,
              ${updatedAtSql}::timestamptz
            )
            ON CONFLICT (id) DO UPDATE SET
              version = EXCLUDED.version,
              status = EXCLUDED.status,
          ...
          console.warn(`   ⚠️  Could not save booking segment ${segmentBookingId}: ${error.message}`)
        }
      }
