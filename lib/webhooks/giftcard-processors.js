      ...
      if (referrer) {
        ...
        if (rewardAlreadyExists) {
          ...
          await saveApplicationLog(prisma, {
            organization_id: organizationId,
            log_type: 'REWARD_PROCESSING',
            log_id: `reward-skip-${customerId}-${Date.now()}`,
            status: 'completed',
            payload: { 
              customerId, 
          ...
          await saveApplicationLog(prisma, {
            organization_id: organizationId,
            log_type: 'REWARD_PROCESSING',
            log_id: `reward-start-${customerId}-${Date.now()}`,
            status: 'processing',
        ...
        if (referrerData && referrerData.length > 0) {
          ...
          if (!rewardAlreadyExists) {
            ...
            try {
              await saveApplicationLog(prisma, {
                organization_id: organizationId,
                log_type: 'REWARD_LOCK',
                log_id: `lock-attempt-${rewardId}`,
                status: 'processing',
            ...
            if (!skipSquareCall) {
              ...
              if (!referrerInfo.gift_card_id) {
                ...
                await saveApplicationLog(prisma, {
                  organization_id: organizationId,
                  log_type: 'SQUARE_GIFT_CARD_API',
                  log_id: `create-gc-${rewardId}`,
                ...
                if (referrerGiftCard?.giftCardId) {
                  ...
                  await saveApplicationLog(prisma, {
                    organization_id: organizationId,
                    log_type: 'REWARD_FINAL_STATUS',
                    log_id: `reward-paid-${rewardId}`,
                    status: 'completed',
                ...
                if (loadResult.success) {
                  ...
                  await saveApplicationLog(prisma, {
                    organization_id: organizationId,
                    log_type: 'REWARD_FINAL_STATUS',
                    log_id: `reward-paid-load-${rewardId}`,
                    status: 'completed',
      ...
      if (referrer) {
        ...
        if (isSelfReferral) {
          ...
          await saveApplicationLog(prisma, {
            organization_id: organizationId,
            log_type: 'REWARD_PROCESSING',
            log_id: `reward-self-booking-${customerId}-${Date.now()}`,
            status: 'completed',
            payload: { customerId, referrerId: referrer.square_customer_id, reason: 'self_referral' }
          })
          return
