...
} = require('../utils/acquisition-source')
...
async function generateUniquePersonalCode(customerName, customerId, organizationId, maxAttempts = 10) {
  ...
  while (attempt < maxAttempts) {
    ...
    if (attempt > 0) {
      ...
      // Check if this code exists within the organization
 ...
 * Used in the booking flow when a merged customer is detected.
 */
async function consolidateMergedCustomerInBooking(organizationId, newCustomerId, emailAddress) {
  try {
    ...
    if (!oldRecords || oldRecords.length === 0) {
      console.log(`   No old records found for email ${emailAddress}`)
      return
    }

    const old = oldRecords[0]
    console.log(`🔀 Found old customer record: ${old.square_customer_id}`)
    console.log(`   personal_code: ${old.personal_code}, got_signup_bonus: ${old.got_signup_bonus}`)
    console.log(`   activated_as_referrer: ${old.activated_as_referrer}, used_referral_code: ${old.used_referral_code}`)
    ...
    console.log(`✅ Consolidated referral data from ${old.square_customer_id} → ${newCustomerId}`)
  } catch (error) {
    console.error(`⚠️ Error consolidating merged customer in booking flow:`, error.message)
    // Non-fatal
  }
}
...
async function sendReferralCodeToNewClient(customerId, customerName, email, phoneNumber, organizationId) {
  try {
...
async function processCustomerCreated(customerData, request, runContext = {}) {
  try {
    ...
    // Ensure ReferralProfile exists for the new customer
    try {
      await prisma.referralProfile.upsert({
        ...
        create: {
          organization_id: organizationId,
          square_customer_id: customerId,
          activated_as_referrer: false
        }
      })
      console.log(`✅ ReferralProfile ensured for customer ${customerId}`)
    } catch (profileError) {
      console.warn(`⚠️ Failed to ensure ReferralProfile: ${profileError.message}`)
    }
...
async function processPaymentCompletion(paymentData, runContext = {}) {
  ...
  try {
    ...
    // 1. Resolve organization_id from payment location or customer record
    let organizationId = null
    const paymentLocationId = paymentData.location_id || paymentData.locationId
    
    if (paymentLocationId) {
      console.log(`📍 Payment attributed to Square location: ${paymentLocationId}`)
      const locCheck = await prisma.$queryRaw`
        SELECT organization_id FROM locations 
        WHERE square_location_id = ${paymentLocationId} 
        LIMIT 1
      `
      organizationId = locCheck?.[0]?.organization_id || null
      if (organizationId) {
    ...
    if (!organizationId) {
      // Fallback: get organization_id from existing customer record
      ...
      if (organizationId) {
        console.log(`✅ Resolved organization_id from customer record: ${organizationId}`)
      }
    }

    if (!organizationId) {
      // Final fallback: use getOrganizationId helper (merchant_id or default org)
      organizationId = await getOrganizationId(runContext?.context || {})
      if (organizationId) {
        console.log(`✅ Resolved organization_id from general context/default: ${organizationId}`)
      }
    }
    ...
    const customerData = await prisma.$queryRaw`
      SELECT square_customer_id, used_referral_code, got_signup_bonus, gift_card_id,
             given_name, family_name, email_address, personal_code, first_payment_completed,
             creation_source, merged_from_customer_id, organization_id
      FROM square_existing_clients
      WHERE square_customer_id = ${customerId}
    ...
    if (!customer) {
      console.warn(`⚠️ Customer ${customerId} not found in square_existing_clients for org ${organizationId}`)
      return
    }
    ...
    if (customer.used_referral_code) {
      ...
      if (referrer) {
        ...
        // STRICT VALIDATION: Prevent self-referral (also handles Square customer merges where IDs differ)
        const isSelfReferral =
          referrer.square_customer_id === customerId ||
          (referrer.email_address && customer.email_address &&
           referrer.email_address.toLowerCase() === customer.email_address.toLowerCase()) ||
          (customer.used_referral_code && customer.personal_code &&
           customer.personal_code.toUpperCase() === customer.used_referral_code.toUpperCase()) ||
          (customer.merged_from_customer_id &&
           customer.merged_from_customer_id === referrer.square_customer_id)
        if (isSelfReferral) {
          console.log(`⚠️ Skipping reward processing: Customer ${customerId} used their own referral code (self-referral)`)
          console.log(`   Referrer ID: ${referrer.square_customer_id}, Customer ID: ${customerId}`)
          console.log(`   Referrer email: ${referrer.email_address}, Customer email: ${customer.email_address}`)
          console.log(`   Creation source: ${customer.creation_source}, Merged from: ${customer.merged_from_customer_id}`)
        ...
        // Check for ANY status (PENDING or PAID) to prevent duplicate processing
        const existingReward = await prisma.$queryRaw`
          SELECT id, status FROM referral_rewards
        ...
        const rewardAlreadyExists = existingReward && existingReward.length > 0
        const rewardAlreadyPaid = rewardAlreadyExists && existingReward[0].status === 'PAID'
        
        if (rewardAlreadyExists) {
          if (rewardAlreadyPaid) {
            console.log(`   ℹ️ Referrer reward already PAID for this friend, skipping duplicate reward`)
          } else {
            console.log(`   ℹ️ Referrer reward is currently PENDING/PROCESSING for this friend, skipping duplicate trigger`)
          }
          
          await saveApplicationLog(prisma, {
            organizationId: organizationId,
            logType: 'REWARD_PROCESSING',
            logId: `reward-skip-${customerId}-${Date.now()}`,
            status: 'completed',
            payload: { 
              customerId, 
              referrerId: referrer.square_customer_id, 
              reason: rewardAlreadyPaid ? 'already_paid' : 'already_pending' 
            }
          })
        } else {
          ...
          await saveApplicationLog(prisma, {
            organizationId: organizationId,
            logType: 'REWARD_PROCESSING',
            logId: `reward-start-${customerId}-${Date.now()}`,
            status: 'processing',
            payload: { customerId, referrerId: referrer.square_customer_id }
          })
        }
        ...
        if (referrerData && referrerData.length > 0) {
          ...
          // Skip reward processing if it was already given OR is currently pending
          if (!rewardAlreadyExists) {
            const rewardAmountCents = 1000
            const rewardId = crypto.randomUUID()
            let skipSquareCall = false

            // ATOMIC STEP 1: Create ReferralReward record with status PENDING
            // This acts as a lock. If another process tries to do this for the same friend, it will fail.
            try {
              await saveApplicationLog(prisma, {
                organizationId: organizationId,
                logType: 'REWARD_LOCK',
                logId: `lock-attempt-${rewardId}`,
                status: 'processing',
                payload: { rewardId, customerId, referrerId: referrer.square_customer_id }
              })

              await prisma.$transaction(async (tx) => {
                // Ensure ReferralProfile exists
                ...
                // Create the reward record as PENDING
                await tx.referralReward.create({
                  data: {
                    ...
                    metadata: { stage: 'created_pending' }
                  }
                })
              })
              console.log(`   🔒 Locked referral reward record (ID: ${rewardId})`)
              
              await saveApplicationLog(prisma, {
                organizationId: organizationId,
                logType: 'REWARD_LOCK',
                logId: `lock-success-${rewardId}`,
                status: 'completed',
                payload: { rewardId, status: 'locked' }
              })
            } catch (lockError) {
              if (lockError.code === 'P2002') {
                console.log(`   ℹ️ Race condition caught: Reward already being processed or finished by another worker. Skipping.`)
                
                await saveApplicationLog(prisma, {
                  organizationId: organizationId,
                  logType: 'REWARD_LOCK',
                  logId: `lock-race-${rewardId}`,
                  status: 'completed',
                  payload: { rewardId, status: 'race_condition_caught' }
                })
                skipSquareCall = true
              } else {
                console.error(`   ❌ Failed to lock referral reward:`, lockError.message)
                
                await saveApplicationLog(prisma, {
                  organizationId: organizationId,
                  logType: 'REWARD_LOCK',
                  logId: `lock-error-${rewardId}`,
                  status: 'error',
                  payload: { rewardId, error: lockError.message }
                })
                throw lockError // Rethrow to let the job retry if it's a transient DB error
              }
            }

            if (!skipSquareCall) {
              let rewardResult = null
              let rewardHadError = false
              ...
              if (!referrerInfo.gift_card_id) {
                ...
                const referrerGiftCardOptions = {
                  ...
                  idempotencyKeySeed: runContext?.correlationId
                    ...
                    : `reward-${rewardId}` // Use rewardId as fallback seed
                }
                ...
                await saveApplicationLog(prisma, {
                  organizationId: organizationId,
                  logType: 'SQUARE_GIFT_CARD_API',
                  logId: `create-gc-${rewardId}`,
                  status: referrerGiftCard?.giftCardId ? 'completed' : 'error',
                  payload: { rewardId, squareResponse: referrerGiftCard }
                })

                if (referrerGiftCard?.giftCardId) {
                  rewardResult = referrerGiftCard
                  
                  // ATOMIC STEP 2: Update reward to PAID
                  // Look up gift_cards DB record by Square ID (gift_card_id is a FK to gift_cards.id)
                  const gcRecord = await prisma.giftCard.findFirst({
                    where: {
                      organization_id: organizationId,
                  ...
                  await prisma.referralReward.update({
                    ...
                    data: {
                      status: 'PAID',
                      gift_card_id: gcRecord?.id || null,
                      paid_at: new Date(),
                      metadata: { square_response: referrerGiftCard, gift_card_square_id: referrerGiftCard.giftCardId }
                    }
                  })

                  await saveApplicationLog(prisma, {
                    organizationId: organizationId,
                    logType: 'REWARD_FINAL_STATUS',
                    logId: `reward-paid-${rewardId}`,
                    status: 'completed',
                    payload: { rewardId, status: 'PAID', giftCardId: referrerGiftCard.giftCardId }
                  })
                ...
                const loadResult = await loadGiftCard(
                  ...
                  runContext?.correlationId
                    ...
                    : { idempotencyKeySeed: `load-${rewardId}` }
                )

                await saveApplicationLog(prisma, {
                  organizationId: organizationId,
                  logType: 'SQUARE_GIFT_CARD_API',
                  logId: `load-gc-${rewardId}`,
                  status: loadResult.success ? 'completed' : 'error',
                  payload: { rewardId, squareResponse: loadResult }
                })

                if (loadResult.success) {
                  rewardResult = loadResult

                  // ATOMIC STEP 2: Update reward to PAID
                  // Look up gift_cards DB record by Square ID (gift_card_id is a FK to gift_cards.id)
                  const gcRecordLoad = await prisma.giftCard.findFirst({
                    where: {
                      organization_id: organizationId,
                  ...
                  await prisma.referralReward.update({
                    ...
                    data: {
                      status: 'PAID',
                      gift_card_id: gcRecordLoad?.id || null,
                      paid_at: new Date(),
                      metadata: { square_response: loadResult, gift_card_square_id: referrerInfo.gift_card_id }
                    }
                  })
                  ...
                  await saveApplicationLog(prisma, {
                    organizationId: organizationId,
                    logType: 'REWARD_FINAL_STATUS',
                    logId: `reward-paid-load-${rewardId}`,
                    status: 'completed',
                    payload: { rewardId, status: 'PAID', giftCardId: referrerInfo.gift_card_id }
                  })
              ...
              if (rewardHadError) {
                ...
                await saveApplicationLog(prisma, {
                  organizationId: organizationId,
                  logType: 'REWARD_FINAL_STATUS',
                  logId: `reward-failed-${rewardId}`,
                  status: 'error',
                  payload: { rewardId, status: 'FAILED' }
                })

                // Update reward to FAILED
                await prisma.referralReward.update({
                  where: { id: rewardId },
                  data: {
                    status: 'FAILED',
                    metadata: { error: 'Square API call failed' }
                  }
                })

                if (runContext?.correlationId) {
                  await markGiftCardRunError(prisma, runContext.correlationId, 'Failed to process referrer reward', {
  ...
  try {
    ...
    let customerExists = await prisma.$queryRaw`
      SELECT square_customer_id, got_signup_bonus, used_referral_code, email_address,
             given_name, family_name, phone_number, personal_code, organization_id,
             creation_source, merged_from_customer_id,
             gift_card_order_id, gift_card_line_item_uid, gift_card_delivery_channel,
             gift_card_activation_url, gift_card_pass_kit_url, gift_card_digital_email
      FROM square_existing_clients
      WHERE square_customer_id = ${customerId}
    ...
    if (!customerExists || customerExists.length === 0) {
      ...
      try {
        ...
        const squareCreationSource = cleanValue(squareCustomer.creationSource || squareCustomer.creation_source)
        ...
        console.log(`✅ Added new customer from booking webhook (creationSource: ${squareCreationSource || 'unknown'})`)

        // If merged customer, consolidate referral data from old record
        if (squareCreationSource === 'MERGE' && squareEmail) {
          console.log(`🔀 Merged customer detected in booking flow: ${customerId}`)
          await consolidateMergedCustomerInBooking(organizationId, customerId, squareEmail)
        }
    ...
    if (!referralCode) {
      ...
      if (!referralCode) {
        ...
        // Extract "How do you know about ZORINA Nail Studio?" acquisition source
        const acquisitionSource = resolveAcquisitionSource(attributes)
        if (acquisitionSource) {
          console.log(`   📊 Acquisition source: "${acquisitionSource}"`)
          await saveAcquisitionSourceIfMissing(prisma, organizationId, customerId, acquisitionSource)
        }
        ...
        if (!referralCode) {
          ...
          for (const [key, value] of Object.entries(attributes)) {
            // Skip the customer's own personal referral code attribute
            if (key === REFERRAL_CODE_ATTRIBUTE_KEY) {
              console.log(`   ⏭️ Skipping key "${key}" - this is the customer's own personal referral code`)
              continue
            }
            if (typeof value === 'string' && value.length > 0) {
    ...
    // If referral code was found before Step 3b ran, still capture acquisition source
    if (referralCode) {
      await saveAcquisitionSourceIfMissing(prisma, organizationId, customerId, 'Friend referral')
    }
      ...
      if (referrer) {
        ...
        const isSelfReferral =
          ...
          (customer.personal_code &&
           customer.personal_code.toUpperCase() === referralCode.toUpperCase()) ||
          (customer.merged_from_customer_id &&
           customer.merged_from_customer_id === referrer.square_customer_id)
        if (isSelfReferral) {
          console.log(`⚠️ Skipping reward processing: Customer ${customerId} used their own referral code (self-referral)`)
          console.log(`   Referrer ID: ${referrer.square_customer_id}, Customer ID: ${customerId}`)
          console.log(`   Referrer email: ${referrer.email_address}, Customer email: ${customer.email_address}`)
          console.log(`   Customer personal_code: ${customer.personal_code}, Referral code used: ${referralCode}`)
          console.log(`   Creation source: ${customer.creation_source}, Merged from: ${customer.merged_from_customer_id}`)

          await saveApplicationLog(prisma, {
            organizationId: organizationId,
            logType: 'REWARD_PROCESSING',
            logId: `reward-self-booking-${customerId}-${Date.now()}`,
            status: 'completed',
            payload: { customerId, referrerId: referrer.square_customer_id, reason: 'self_referral' }
          })
          return
...
async function processNotificationJob(payload, runContext) {
  const {
    type,
    customerName,
    email,
  ...
  if (type === 'GIFT_CARD_DELIVERY' || type === 'FRIEND_ACTIVATION' || type === 'REFERRER_ACTIVATION') {
    return sendGiftCardEmailNotification({
      customerName,
      email,
      giftCardGan,
      amountCents,
...
// Export the main processor functions and helper functions
module.exports = {
