...
async function resolveGiftCardContext({ gan, prisma, giftCardsApi }) {
  ...
  try {
    ...
    if (giftCardCache) {
      ...
      balanceCents = normalizeCents(giftCardCache.lastBalanceCents)

      try {
        const squareGiftCard = await giftCardsApi.retrieveGiftCard(giftCardCache.squareGiftCardId)
        if (squareGiftCard.result?.giftCard) {
          const currentBalance = normalizeCents(
            squareGiftCard.result.giftCard.balanceMoney?.amount
          )
          balanceCents = currentBalance
          giftCardInfo = squareGiftCard.result.giftCard
        }
      } catch (squareError) {
        console.warn(`⚠️ Could not fetch current balance from Square, using cached: ${squareError.message}`)
      }
    } else {
