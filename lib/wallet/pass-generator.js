...
async function generateGiftCardPass({
  ...
  try {
    ...
    // CRITICAL: Ensure webServiceURL and authenticationToken are in the final pass
    // Passkit-generator may strip these during validation, so we need to ensure they're present
    if (webServiceUrl) {
      const authToken = generateAuthToken(serialNumber)
      
      // Try to use PKPass methods if available
      if (typeof pass.setWebServiceURL === 'function') {
        ...
        console.log(`ℹ️ PKPass.setWebServiceURL() not available, will manually inject pass.json`)
      }
      ...
      if (typeof pass.setAuthenticationToken === 'function') {
        ...
        console.log(`ℹ️ PKPass.setAuthenticationToken() not available, will manually inject pass.json`)
      }
      
      // CRITICAL: Since PKPass methods may not exist, manually ensure pass.json has webServiceURL
      // Try to access and update pass data directly
      let passJsonUpdated = false
      try {
        // Try different ways to access the pass data
        if (pass.pass && typeof pass.pass === 'object') {
          pass.pass.webServiceURL = webServiceUrl
          pass.pass.authenticationToken = authToken
          passJsonUpdated = true
          console.log(`✅ Manually set webServiceURL in pass.pass object`)
        } else if (pass._pass && typeof pass._pass === 'object') {
          pass._pass.webServiceURL = webServiceUrl
          pass._pass.authenticationToken = authToken
          passJsonUpdated = true
          console.log(`✅ Manually set webServiceURL in pass._pass object`)
        }
      } catch (e) {
        console.warn(`⚠️ Could not access pass data directly: ${e.message}`)
      }
      
      // If we couldn't update pass data directly, inject pass.json via addBuffer
      // This ensures webServiceURL is definitely in the final .pkpass file
      if (!passJsonUpdated) {
        console.log(`🔄 Manually injecting pass.json with webServiceURL via addBuffer...`)
        try {
          // Ensure passPayload has webServiceURL (it should already)
          if (!passPayload.webServiceURL) {
            passPayload.webServiceURL = webServiceUrl
          }
          if (!passPayload.authenticationToken) {
            passPayload.authenticationToken = authToken
          }
          
          // Create new pass.json buffer with webServiceURL guaranteed
          const finalPassJsonBuffer = Buffer.from(JSON.stringify(passPayload, null, 2))
          pass.addBuffer('pass.json', finalPassJsonBuffer)
          console.log(`✅ Manually injected pass.json with webServiceURL (${finalPassJsonBuffer.length} bytes)`)
          console.log(`   webServiceURL: ${webServiceUrl}`)
          console.log(`   authenticationToken: ${authToken.substring(0, 10)}...`)
        } catch (injectError) {
          console.error(`❌ Failed to inject pass.json: ${injectError.message}`)
          console.error(`   This may prevent Apple Wallet registration!`)
        }
      }
    }
    ...
    // Final verification: Check if webServiceURL is in the generated .pkpass buffer
    // Since .pkpass is a zip file containing pass.json, we can search for the string
    if (webServiceUrl) {
      const bufferString = buffer.toString('binary')
      if (bufferString.includes('webServiceURL')) {
        // Try to extract the actual URL value for verification
        const urlMatch = bufferString.match(/webServiceURL["\s]*[:=]["\s]*([^"}\s]+)/)
        if (urlMatch) {
          console.log(`✅ FINAL VERIFICATION: webServiceURL found in .pkpass file: ${urlMatch[1]}`)
        } else {
          console.log(`✅ FINAL VERIFICATION: webServiceURL key found in .pkpass file`)
        }
        
        // Also verify authenticationToken is present
        if (bufferString.includes('authenticationToken')) {
          console.log(`✅ FINAL VERIFICATION: authenticationToken found in .pkpass file`)
        } else {
          console.warn(`⚠️ WARNING: authenticationToken NOT found in .pkpass file`)
        }
      } else {
        console.error(`❌ CRITICAL: webServiceURL NOT found in final .pkpass buffer!`)
        console.error(`   The pass was generated but webServiceURL may have been stripped`)
        console.error(`   This pass will NOT register properly with Apple Wallet`)
        console.error(`   Apple will NOT call the registration endpoint!`)
      }
    }
