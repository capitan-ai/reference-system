...
async function generateGiftCardPass({
  ...
  try {
    ...
    if (webServiceUrl) {
      ...
      // Verify and ensure webServiceURL is preserved after PKPass instantiation
      // The passkit-generator library may strip webServiceURL during validation,
      // so we need to ensure it's properly set using the library's internal structure
      try {
        let passJsonAfterCreation = null
        let webServiceUrlPresent = false
        
        // Try to access pass data directly (passkit-generator v3+ structure)
        // The library stores pass.json data in different ways depending on version
        if (pass.pass && typeof pass.pass === 'object') {
          passJsonAfterCreation = pass.pass
        } else if (pass._pass && typeof pass._pass === 'object') {
          // Some versions use _pass (private property)
          passJsonAfterCreation = pass._pass
        } else if (typeof pass.toJSON === 'function') {
          // Some versions have toJSON method
          passJsonAfterCreation = pass.toJSON()
        }
        
        // Check if webServiceURL is present
        if (passJsonAfterCreation) {
          webServiceUrlPresent = !!passJsonAfterCreation.webServiceURL
          
          if (webServiceUrlPresent) {
            console.log(`✅ VERIFIED: webServiceURL still present after PKPass creation: ${passJsonAfterCreation.webServiceURL}`)
          } else {
            // webServiceURL was in pass.json but is missing after PKPass creation
            // This can happen if the library strips it during validation
            console.warn(`⚠️ WARNING: webServiceURL missing from pass after PKPass creation!`)
            console.warn(`   Re-adding webServiceURL directly to pass data...`)
            
            // Manually add it to the pass data structure
            const authToken = generateAuthToken(serialNumber)
            if (pass.pass) {
              pass.pass.webServiceURL = webServiceUrl
              pass.pass.authenticationToken = authToken
              console.log(`✅ Manually added webServiceURL to pass.pass object`)
            } else if (pass._pass) {
              pass._pass.webServiceURL = webServiceUrl
              pass._pass.authenticationToken = authToken
              console.log(`✅ Manually added webServiceURL to pass._pass object`)
            }
            
            // Verify it was added
            if (passJsonAfterCreation.webServiceURL) {
              console.log(`✅ CONFIRMED: webServiceURL now present: ${passJsonAfterCreation.webServiceURL}`)
            } else {
              console.error(`❌ ERROR: Failed to add webServiceURL to pass data structure`)
              console.error(`   Pass may not register with Apple Wallet properly`)
            }
          }
        } else {
          // If we couldn't access pass data directly, webServiceURL should still be preserved
          // since it was in the original pass.json buffer passed to PKPass constructor
          console.log(`ℹ️ Could not access pass data directly for verification`)
          console.log(`   However, webServiceURL was set in pass.json before PKPass creation`)
          console.log(`   If setWebServiceURL() method exists and was called, it should be preserved`)
        }
      } catch (e) {
        console.warn(`⚠️ Could not verify pass contents after PKPass creation: ${e.message}`)
        console.warn(`   However, webServiceURL was set in pass.json before PKPass creation`)
      }
    }
