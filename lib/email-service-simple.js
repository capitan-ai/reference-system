...
/**
 * Track notification in notification_events table
 * @param {Object} params
 * @param {string} params.channel - 'EMAIL' or 'SMS'
 * @param {string} params.templateType - 'REFERRAL_INVITE', 'FRIEND_ACTIVATION', 'REFERRER_ACTIVATION', 'OTHER'
 * @param {string} params.status - 'sent', 'failed', 'queued', 'skipped'
 * @param {string} params.customerId - Square customer ID
 * @param {string} params.referrerCustomerId - Referrer customer ID (optional)
 * @param {string} params.externalId - SendGrid/Twilio message ID
 * @param {string} params.templateId - Template ID (optional)
 * @param {Object} params.metadata - Additional metadata
 * @param {Date} params.sentAt - When notification was sent (optional)
 * @param {string} params.errorMessage - Error message if failed (optional)
 * @param {string} params.errorCode - Error code if failed (optional)
...
async function trackNotification({
  ...
  try {
    ...
    if (!resolvedOrgId && customerId) {
      ...
      resolvedOrgId = customer?.organization_id
    }

    if (!resolvedOrgId) {
      console.warn(`⚠️ Cannot track notification: organizationId could not be resolved for customer ${customerId}`)
      return
    }
    ...
    await prisma.notificationEvent.create({
      data: {
        ...
        templateId,
        metadata: metadata || {},
        sentAt: sentAt ? new Date(sentAt) : (status === 'sent' ? new Date() : null),
        statusAt: new Date(),
        errorMessage,
        errorCode,
        organization: {
    ...
    console.log(`✅ Notification tracked: ${channel} ${templateType} to ${customerId || 'N/A'} - ${status}`)
  } catch (error) {
    // Don't throw - tracking failure shouldn't break the main process
    console.error(`⚠️ Failed to track notification:`, error.message)
  }
}

// Keep trackEmailNotification for backward compatibility
async function trackEmailNotification(params) {
  return trackNotification({ ...params, channel: 'EMAIL' })
}
...
async function sendGiftCardIssuedEmail(customerName, email, payload, options = {}) {
  ...
  try {
    ...
    // Track email notification
    const templateType = payload?.role === 'friend' ? 'FRIEND_ACTIVATION' : 'REFERRER_ACTIVATION'
    await trackEmailNotification({
      channel: 'EMAIL',
      templateType,
      status: 'sent',
