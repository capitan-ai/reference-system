...
/**
 * Track email notification in notification_events table
 * @param {Object} params
 * @param {string} params.channel - 'EMAIL'
 * @param {string} params.templateType - 'REFERRAL_INVITE', 'FRIEND_ACTIVATION', 'REFERRER_ACTIVATION', 'OTHER'
 * @param {string} params.status - 'sent', 'failed', 'queued'
 * @param {string} params.customerId - Square customer ID
 * @param {string} params.referrerCustomerId - Referrer customer ID (optional)
 * @param {string} params.externalId - SendGrid message ID
 * @param {string} params.templateId - SendGrid template ID (optional)
 * @param {Object} params.metadata - Additional metadata
 * @param {Date} params.sentAt - When email was sent (optional)
...
async function trackEmailNotification({
  ...
  try {
    ...
    if (!resolvedOrgId) {
      console.warn(`⚠️ Cannot track email notification: organizationId could not be resolved for customer ${customerId}`)
      return
    }
    ...
    console.log(`✅ Email notification tracked: ${templateType} to ${customerId || 'N/A'} - ${status}`)
  } catch (error) {
    // Don't throw - tracking failure shouldn't break email sending
    console.error(`⚠️ Failed to track email notification:`, error.message)
  }
}
