#!/usr/bin/env node
/**
 * Backfill orders from last 30 days using Square REST API directly (no SDK)
 * 
 * Usage: node scripts/backfill-last-30-days-api.js
 */
...
console.log('🔄 Backfilling Orders from LAST 30 DAYS (using Square API)\n');
...
async function processOrder(orderId, locationId, orderNum) {
  ...
  try {
    ...
    console.log(`   ❌ API error: ${err.message.substring(0, 50)}`);
  ...
  for (const item of lineItems) {
    ...
    try {
      ...
      // Silently continue on line item errors
    }
  }
...
async function backfillOrders() {
  ...
  for (const location of locations) {
    ...
    do {
      ...
      console.log(`   Batch ${batchNum}: ${orders.length} orders`);
      ...
      for (const orderSummary of orders) {
        ...
        const dateStr = orderSummary.created_at ? new Date(orderSummary.created_at).toLocaleDateString() : '';

        const result = await processOrder(orderId, locationId, processedOrders);
        ...
        if (processedOrders % 50 === 0) {
          console.log(`   📊 Progress: ${processedOrders} orders processed (${newOrders} new, ${updatedOrders} updated)`);
        }
    ...
    console.log(`   ✅ Location complete: ${locationOrderCount} orders`);
  }

  console.log('\n' + '='.repeat(60));
  console.log('\n📊 BACKFILL SUMMARY (Last 30 Days):');
  console.log('='.repeat(60));
