...
// Send test emails to Bozhena V and Iana Zorina
...
async function sendEmails() {
  try {
    console.log('📧 Sending test emails to Bozhena V and Iana Zorina')
    ...
    for (const customer of CUSTOMERS) {
      ...
      if (localCustomer && localCustomer.RefLinks.length > 0) {
        referralCode = localCustomer.RefLinks[0].refCode
        referralUrl = localCustomer.RefLinks[0].url || `https://referral-system-salon.vercel.app/ref/${referralCode}`
        console.log(`   ✅ Found in local DB with referral code: ${referralCode}`)
      } else {
        // Generate a test referral code
        referralCode = `TEST-${customer.squareId.slice(-6).toUpperCase()}`
        referralUrl = `https://referral-system-salon.vercel.app/ref/${referralCode}`
        console.log(`   ⚠️  Not in local DB, using test referral code: ${referralCode}`)
      }
          ...
          <body style="margin:0;padding:0;background:#F2EBDD;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#333;">
            <div style="max-width:700px;margin:0 auto;background:#F2EBDD;padding:10px;">
              ...
              <div style="text-align:center;margin-bottom:25px;padding:0 10px;">
                <h1 style="font-size:18px;color:#333;font-weight:500;margin-bottom:12px;">Hi ${customer.name}! 👋</h1>
                  ...
                  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:640px;background:#F2EBDD;">
                    ...
                    <tr>
                      <td style="background:#FFFFFF;border:2px solid #5C6B50;border-radius:14px;padding:28px;">
                        ...
                        <p style="margin:0 0 16px;font-size:14px;line-height:1.6;text-align:center;">Hi ${customer.name},</p>
