/**
 * Backfill orders from January 26-28, 2026
 * Specifically targeting the missing orders during this period
 * 
 * Usage: node scripts/backfill-jan-26-28-orders.js
 */
...
const endDate = new Date('2026-01-29T00:00:00Z') // Up to end of Jan 28

const beginTime = startDate.toISOString()
const endTime = endDate.toISOString()

console.log('🔄 Backfilling Missing Orders (Jan 26-28, 2026)\n')
...
async function processOrder(orderFromSearch) {
  ...
  console.log(`   📍 Location: ${locationId || 'N/A'} | Customer: ${customerId?.substring(0, 8) || 'N/A'}... | ${lineItems.length} items | ${isNew ? 'NEW' : 'UPDATE'}`)
...
async function backfillJan2628Orders() {
  // Get all locations
  ...
  // Check existing orders in DB for this date range
  console.log('\n📋 Step 2: Checking existing orders in database...')
  const existingOrders = await prisma.$queryRaw`
    ...
    WHERE EXISTS (
      ...
      AND oli.order_created_at >= ${startDate}
      AND oli.order_created_at < ${endDate}
    )
  `
  console.log(`   📊 Orders in DB for this period: ${existingOrders[0].count}`)
  ...
  for (const locationId of locationIds) {
    ...
    do {
      ...
      for (const order of orders) {
        ...
        const dateStr = orderDate ? new Date(orderDate).toISOString().split('T')[0] : 'unknown'
        
        console.log(`\n[${processedCount}] Order ${order.id} (${dateStr})`)
  ...
  console.log('\n📊 BACKFILL SUMMARY:')
  console.log('='.repeat(60))
  console.log(`   📦 Orders in Square API (Jan 26-28): ${totalOrdersFromSquare}`)
