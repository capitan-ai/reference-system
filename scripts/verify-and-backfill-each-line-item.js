#!/usr/bin/env node
/**
 * Verify and backfill each line item from Square using retrieveOrder API
 * Based on: https://developer.squareup.com/reference/square/orders-api/retrieve-order
 */
...
try {
  ...
  console.log(`📚 API Reference: https://developer.squareup.com/reference/square/orders-api/retrieve-order\n`)
} catch (error) {
...
async function verifyAndBackfillLineItems() {
  console.log('🔍 Verifying and Backfilling Each Line Item from Square\n')
  console.log('='.repeat(60))

  try {
    // Get all orders that don't have line items in the database
    ...
    console.log(`📊 Found ${ordersWithoutLineItems.length} orders without line items in database\n`)
    ...
    let errors = []
    const BATCH_SIZE = 50 // Smaller batch to avoid rate limits

    console.log(`🔄 Retrieving each order from Square API and checking line items...\n`)
    console.log(`   Using GET /v2/orders/{order_id} endpoint\n`)
    ...
    for (let i = 0; i < ordersWithoutLineItems.length; i += BATCH_SIZE) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (order) => {
          try {
            // Retrieve order from Square API
            ...
            // Get line_items from the order
            // According to Square API docs, line_items is an array
            ...
            if (lineItems.length === 0) {
              return { 
                success: true, 
                orderId: order.order_id, 
                reason: 'no_line_items_in_square',
                lineItemsInSquare: 0,
            ...
            // Extract order totals from Square API response
            ...
            // Process each line item from Square API
            for (const item of lineItems) {
              ...
              try {
                // Extract all money fields from line item
                // Reference: Square API docs show these fields in the response
                ...
                // Prepare line item data according to our schema
                const lineItemData = {
                ...
                // Insert line item using raw SQL with ON CONFLICT
                try {
                  ...
                  itemErrors++
                  if (itemErrors <= 5) {
                    errors.push({ orderId: order.order_id, itemUid: itemUid, error: itemError.message })
                  }
                }
              } catch (itemError) {
                itemErrors++
            ...
            return { 
              ...
              errors: itemErrors,
              lineItemsInSquare: lineItems.length,
              state: orderState
            }
          } catch (apiError) {
            return { 
      ...
      for (const result of batchResults) {
        if (result.success) {
          if (result.lineItemsInSquare === 0) {
            skipped++
          } else {
            successful++
            lineItemsAdded += result.saved || 0
            if (result.saved < result.total) {
              // Some line items failed to save
              console.log(`   ⚠️  ${result.orderId}: Saved ${result.saved}/${result.total} line items (state: ${result.state})`)
            }
          }
        } else {
          failed++
          if (failed <= 10) {
            console.log(`   ❌ ${result.orderId}: ${result.reason || result.error}`)
          }
        }
      }

      if ((i + BATCH_SIZE) % 100 === 0 || i + BATCH_SIZE >= ordersWithoutLineItems.length) {
        const progress = Math.min(i + BATCH_SIZE, ordersWithoutLineItems.length)
        console.log(`   Progress: ${progress}/${ordersWithoutLineItems.length} (${successful} successful, ${skipped} skipped, ${failed} failed)`)
      }
    ...
    console.log(`   ⏭️  Skipped (no line items in Square): ${skipped}`)
    console.log(`   ❌ Failed: ${failed}`)
    console.log(`   📦 Line items added: ${lineItemsAdded}`)

    if (errors.length > 0) {
      console.log(`\n⚠️  Errors encountered (showing first 5):`)
      errors.slice(0, 5).forEach(err => {
        console.log(`   Order ${err.orderId}, Item ${err.itemUid}: ${err.error}`)
      })
    }

    console.log(`\n📚 API Reference: https://developer.squareup.com/reference/square/orders-api/retrieve-order`)
...
verifyAndBackfillLineItems()
  .then(() => {
    console.log('\n✅ Verification and backfill complete')
    process.exit(0)
  })
  .catch((error) => {
    console.error('❌ Verification and backfill failed:', error)
    process.exit(1)
  })
