#!/usr/bin/env node
/**
 * Backfill all missing orders and line items to ensure complete data
 */
...
async function backfillMissing() {
  console.log('🔄 Backfilling Missing Orders and Line Items\n')
  console.log('='.repeat(60))

  try {
    // Get missing orders from 2026
    ...
    console.log(`📊 Found ${missingOrders.length} missing orders from 2026\n`)

    // Also get orders without line items from all years
    const ordersWithoutLineItems = await prisma.$queryRaw`
      ...
      WHERE o.order_id = ANY(${squareOrderIdsArray}::text[])
        AND o.id NOT IN (
          ...
          WHERE order_created_at >= ${startDate2026}
            AND order_created_at < ${endDate2026}
        )
    `

    console.log(`📊 Found ${ordersWithoutLineItems.length} orders without line items from 2026\n`)

    // Combine all orders to process
    const allOrdersToProcess = [
      ...missingOrders.map(o => ({ orderId: o.order_id, type: 'missing' })),
      ...ordersWithoutLineItems.map(o => ({ orderId: o.order_id, type: 'no_line_items' }))
    ]

    const uniqueOrderIds = [...new Set(allOrdersToProcess.map(o => o.orderId))]
    console.log(`🔄 Processing ${uniqueOrderIds.length} orders...\n`)
    ...
    for (let i = 0; i < uniqueOrderIds.length; i += BATCH_SIZE) {
      ...
      for (const result of batchResults) {
        if (result.success) {
          ...
          failed++
          if (failed <= 10) {
            console.log(`   ❌ Failed: ${result.orderId} - ${result.reason || result.error}`)
          }
        }
      }

      if ((i + BATCH_SIZE) % 200 === 0 || i + BATCH_SIZE >= uniqueOrderIds.length) {
        console.log(`   Progress: ${Math.min(i + BATCH_SIZE, uniqueOrderIds.length)}/${uniqueOrderIds.length} (${successful} successful, ${skipped} skipped, ${failed} failed)`)
      }

      // Delay to avoid rate limiting
      if (i + BATCH_SIZE < uniqueOrderIds.length) {
    ...
    console.log('\n' + '='.repeat(60))
    console.log('\n📊 BACKFILL SUMMARY:\n')
    console.log(`   Total orders processed: ${uniqueOrderIds.length}`)
