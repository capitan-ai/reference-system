...
async function comparePaymentsWithSquare() {
  console.log('🔍 Comparing February Payments: Square vs Database\n');

  try {
    // Get all February payments from Square
    console.log('📥 Fetching all February payments from Square API...\n');
    ...
    while (true) {
      ...
      if (pageCount % 5 === 0) {
        console.log(`  Fetched ${allSquarePayments.length} payments so far...`);
      }
    }

    console.log(`✅ Total payments from Square: ${allSquarePayments.length}\n`);

    // Get all February payments from DB
    console.log('📥 Fetching all February payments from database...');
    ...
    const squareOrderIds = new Set(allSquarePayments.filter(p => p.order_id).map(p => p.order_id));
    const dbPaymentIds = new Set(dbPayments.map(p => p.payment_id));
    const dbOrderIds = new Set(dbPayments.filter(p => p.order_id).map(p => p.order_id));
    ...
    const ordersInSquareNotDb = Array.from(squareOrderIds).filter(id => !dbOrderIds.has(id));
    const ordersInDbNotSquare = Array.from(dbOrderIds).filter(id => !squareOrderIds.has(id));
    ...
    console.log('\n' + '═'.repeat(80));
    console.log('\n📦 ORDER IDs:\n');
    console.log(`Square payments with orders: ${squareOrderIds.size}`);
    console.log(`DB payments with orders: ${dbOrderIds.size}`);
    console.log(`Difference: ${squareOrderIds.size - dbOrderIds.size}`);
    console.log();

    if (ordersInSquareNotDb.length > 0) {
      console.log(`❌ MISSING IN DB: ${ordersInSquareNotDb.length} orders from Square not linked in DB`);
      ordersInSquareNotDb.slice(0, 10).forEach((id, idx) => {
        const sqPayment = allSquarePayments.find(p => p.order_id === id);
        console.log(`  ${idx + 1}. ${id.substring(0, 20)}... (payment: ${sqPayment.id.substring(0, 15)}...)`);
      });
      if (ordersInSquareNotDb.length > 10) {
        console.log(`  ... and ${ordersInSquareNotDb.length - 10} more`);
      }
    } else {
      console.log('✅ All orders from Square payments are in DB');
    }

    console.log();

    if (ordersInDbNotSquare.length > 0) {
      console.log(`⚠️  EXTRA IN DB: ${ordersInDbNotSquare.length} orders in DB not in Square payments`);
    } else {
      console.log('✅ No extra orders in DB');
    }
