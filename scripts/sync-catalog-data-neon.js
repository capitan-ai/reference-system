...
const neonUrl = "postgresql://neondb_owner:npg_Ge2VO9JYtLSu@ep-cool-math-ad2tyq7d-pooler.c-2.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require";
...
// Use organization_id from the Neon database if available, or a fallback
const organizationId = 'd0e24178-2f94-4033-bc91-41f22df58278';

async function main() {
  console.log('🚀 Starting catalog sync to Neon...');
  ...
  for (const batch of catalogData) {
    for (const obj of batch.objects) {
      if (obj.type === 'ITEM') {
        ...
        const catalogItem = await prisma.catalog_items.upsert({
        ...
        for (const variation of obj.item_data.variations) {
          ...
          await prisma.catalog_variations.upsert({
  ...
  console.log('✅ Catalog sync to Neon completed!');
}
