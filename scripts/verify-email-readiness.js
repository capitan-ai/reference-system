...
/**
 * Verify that everything is ready to send referral code emails to customers
 */
...
async function verifyEmailReadiness() {
  try {
    console.log('🔍 Verifying Email Readiness for Referral Codes\n')
  ...
  // 3. Check Customers with Referral Codes
  console.log('\n👥 3. Customer Data:')
  try {
    ...
    console.log(`   📊 Customers with email addresses: ${customersWithEmail}`)
    ...
    console.log(`   📊 Customers with active referral links: ${customersWithRefLinks}`)
    
    const customersReadyForEmail = await prisma.customer.count({
      where: {
        email: {
          not: null,
          not: ''
    ...
    console.log(`   📊 Customers ready for emails: ${customersReadyForEmail}`)
    ...
    if (customersReadyForEmail > 0) {
      ...
      sampleCustomers.forEach((customer, i) => {
        ...
        const name = customer.fullName || `${customer.firstName || ''} ${customer.lastName || ''}`.trim() || 'Unknown'
        console.log(`      ${i + 1}. ${name} (${customer.email}) - Code: ${refLink?.refCode || 'N/A'}`)
      })
    }
  ...
  if (allReady && !emailDisabled) {
    console.log('✅ Everything is ready! You can send emails to customers.')
    console.log('\n📧 To send emails, run:')
    console.log('   DRY_RUN=false node scripts/send-referral-emails-to-customers.js')
    console.log('\n⚠️  Or test first with dry run:')
    console.log('   node scripts/send-referral-emails-to-customers.js')
