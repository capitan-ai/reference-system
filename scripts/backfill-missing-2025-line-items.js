#!/usr/bin/env node
/**
 * Backfill missing line items for 2025 orders that have them in Square but not in DB
 */
...
async function backfillLineItems() {
  console.log('🔄 Backfilling Missing 2025 Line Items\n')
  console.log('='.repeat(60))

  try {
    // Find 2025 orders without line items
    ...
    // Find which don't have line items
    ...
    console.log(`📊 Found ${missingLineItems.length} orders without 2025 line items\n`)
    ...
    for (let i = 0; i < missingLineItems.length; i += BATCH_SIZE) {
                  ...
                  try {
                    await prisma.$executeRaw`
                      ...
                      SET 
                        ...
                        updated_at = NOW()
                      WHERE organization_id = ${order.organization_id}::uuid
                        AND uid = ${itemUid}
                    `
                    saved++
                  } catch (updateError) {
      ...
      if ((i + BATCH_SIZE) % 200 === 0 || i + BATCH_SIZE >= missingLineItems.length) {
        console.log(`   Progress: ${Math.min(i + BATCH_SIZE, missingLineItems.length)}/${missingLineItems.length} (${successful} successful, ${skipped} skipped, ${failed} failed)`)
      }

      // Small delay to avoid rate limiting
      if (i + BATCH_SIZE < missingLineItems.length) {
    ...
    console.log('\n' + '='.repeat(60))
    console.log('\n📊 BACKFILL SUMMARY:\n')
    console.log(`   Total orders processed: ${missingLineItems.length}`)
