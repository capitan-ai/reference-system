    ...
    const viewTest = await prisma.$queryRaw`
      WITH manual_count AS (
        SELECT 
          b.organization_id,
          b.location_id,
          DATE(b.start_at AT TIME ZONE 'America/Los_Angeles') as date,
        ...
        WHERE b.status = 'ACCEPTED'
          AND DATE(ca.first_booking_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/Los_Angeles') = 
              DATE(b.start_at AT TIME ZONE 'America/Los_Angeles')
        GROUP BY b.organization_id, b.location_id, DATE(b.start_at AT TIME ZONE 'America/Los_Angeles')
      )
      SELECT 
