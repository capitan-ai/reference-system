...
**Реализация:** `lib/workers/discount-engine-worker.js` — вызывается из cron после `processMasterEarnings`.

### Слой 4: Analytics Layer (Dashboard) ✅
**Триггер:** Расписание (Cron) каждый час.
**Действие:** Агрегация данных из Ledger и Snapshots в таблицу `MasterPerformanceDaily`.
**Метрики:** `available_minutes` (из MasterSettings, default 480), `utilization_rate`, `composite_score`, разбивка по `location_id`.
